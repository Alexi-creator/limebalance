...
// Содержимое письма для рендера в text/html.
type EmailContent = {
  title: string;
  intro: string;
  note: string;
...
export class MailService {
  ...
  // Создаётся лениво при первой отправке и только если задан SMTP_HOST.
  ...
  // Собирает ссылку фронтенда вида {FRONTEND_URL}/path?token=...
  ...
  private async send(to: string, subject: string, content: EmailContent) {
    ...
    // SMTP не настроен — в dev просто логируем содержимое, чтобы ссылкой можно было воспользоваться.
    if (!transporter) {
      this.logger.warn(`SMTP не настроен, письмо не отправлено. To: ${to}\n${text}`);
      return;
    }
    ...
    try {
      ...
      // Падение отправки не должно ронять основной запрос — логируем и продолжаем.
      this.logger.error(`Не удалось отправить письмо на ${to}`, err as Error);
    }
  }
  ...
  // HTML-версия с кнопкой. Стили инлайновые — так их понимают почтовые клиенты.
